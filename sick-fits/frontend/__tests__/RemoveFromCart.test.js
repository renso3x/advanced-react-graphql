...
import RemoveCartitem, { REMOVE_ITEM_MUTATION } from '../components/RemoveCartitem';
...
const mocks = [
  ...
  {
    request: { query: REMOVE_ITEM_MUTATION, variables: { id: 'abc123' } },
    result: {
      data: {
        removeToCart: {
          __typename: 'CartItem',
    ...
    const wrapper = mount(
      <MockedProvider mocks={mocks}>
        <ApolloConsumer>
          {client => {
            apolloClient = client;
            return <RemoveCartitem id="abc123" />
          }}
        </ApolloConsumer>
      </MockedProvider>
    );
